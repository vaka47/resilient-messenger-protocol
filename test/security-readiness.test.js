...
import { registerStateWithApi } from "../src/client/workflow.js";
...
test("key transparency records device lifecycle in a verifiable hash chain", async () => {
  ...
  alicePhone = await registerStateWithApi({
  ...
  assert.equal(transparency.entries[0].type, "device.registered");
